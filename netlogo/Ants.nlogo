...
to setup
  ...
  if food-x = 0  [ set food-x [] ]
  if food-y = 0  [ set food-y [] ]
  setup-patches
  reset-ticks
end
...
to setup-nest  ;; patch procedure
  ...
  set nest? (distancexy nest-x nest-y) < 5
...
to setup-food  ;; patch procedure
  ...
  ;;while [counter < pile-amount] [(random (world-width / 2)) * one-of [1 -1]]

  (foreach food-x food-y
    [ if (distancexy ?1 ?2) < 5
      [ set food-source-number 1 ]])
